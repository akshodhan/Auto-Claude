...
PROVIDER_MODELS = {
    ...
    AIProvider.GROQ: {
        "llama-4-maverick-17b-128e-instruct": AIModelConfig(
            AIProvider.GROQ, "llama-4-maverick-17b-128e-instruct", 128000, True, True
        ),
        "llama-4-scout-17b-16e-instruct": AIModelConfig(
            AIProvider.GROQ, "llama-4-scout-17b-16e-instruct", 128000, True, True
        ),
        "qwen-qwq-32b": AIModelConfig(
            AIProvider.GROQ, "qwen-qwq-32b", 32768, True, True
        ),
        "deepseek-r1-distill-llama-70b": AIModelConfig(
            AIProvider.GROQ, "deepseek-r1-distill-llama-70b", 32768, True, True
        ),
...
def create_ai_client(
    ...
    **kwargs
    ...
    # For OpenAI-compatible providers (OpenAI, DeepSeek, Groq, OpenRouter, GitHub Copilot, Azure OpenAI)
    if provider in [AIProvider.OPENAI, AIProvider.DEEPSEEK, AIProvider.GROQ, AIProvider.OPENROUTER, AIProvider.GITHUB_COPILOT, AIProvider.AZURE_OPENAI]:
    ...
    # For AWS Bedrock (special handling)
    if provider == AIProvider.AWS_BEDROCK:
        return create_aws_bedrock_client(
            model_id=model_id,
            **kwargs
        )
...
def create_aws_bedrock_client(
    ...
    **kwargs
    ...
    try:
        ...
        raise ImportError(
            "boto3 library is required for AWS Bedrock. "
